...
use smallvec::SmallVec;
use savefile::{WithSchema, Serialize, Deserialize, Introspect, Packed};
...
pub trait TreeNodeItem {
    ...
    /// or even Self.
    type Key : PartialEq+Eq+Hash+WithSchema+Packed+Serialize+Deserialize+Introspect;
...
struct TreeNode<T:WithSchema + Serialize + Deserialize+Introspect+Packed+'static> {
    bb: BB,
    sub_cell_size: i32,
    sub_cell_shift: u32,
    /// Terminology:
...
#[derive(Savefile,Clone,Debug)]
pub struct QuadBTree<T:TreeNodeItem + WithSchema+Packed + Serialize + Deserialize+Introspect+Packed+'static> {
...
impl<T:TreeNodeItem + WithSchema + Packed + Serialize + Deserialize+Introspect+Packed + Clone> QuadBTree<T> {
...
impl<T:TreeNodeItem + WithSchema + Packed + Serialize + Deserialize+Introspect+Packed> QuadBTree<T> {
...
mod tests {
    ...
    pub(crate) fn for_test_get_first_free<T:TreeNodeItem+WithSchema+Serialize+Deserialize+Introspect+Packed>(tree:&QuadBTree<T>) -> Option<u32> {
        tree.first_free
    }
